...
def get_sum_amount(dict_transactions: List[Dict[str, Any]]) -> float:
    ...
    rub_sum = round(sum(float(transactions["operationAmount"]["amount"]) for transactions in data_transactions if
                        "operationAmount" in transactions and transactions["operationAmount"]["currency"][
                            "code"] == "RUB"), 2)

    sum_usd = round(sum(float(transactions["operationAmount"]["amount"]) for transactions in data_transactions if
                        "operationAmount" in transactions and transactions["operationAmount"]["currency"][
                            "code"] == "USD"), 2)

    sum_eur = round(sum(float(transactions["operationAmount"]["amount"]) for transactions in data_transactions if
                        "operationAmount" in transactions and transactions["operationAmount"]["currency"][
                            "code"] == "EUR"), 2)
