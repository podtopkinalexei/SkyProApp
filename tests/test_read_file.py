...
def test_read_csv_file(mock_sniffer, mock_open, mock_read_csv):
    ...
    test_df = pd.DataFrame(
        {"id": [650703], "status": ["EXECUTED"], "date": ["2023-09-05T11:30:32Z"]}
    )
...
@patch(
    ...
    read_data=json.dumps([{"id": 650703, "status": "EXECUTED"}]),
)
@patch("json.load")
...
def test_empty_file(mock_read_csv):
    ...
    assert (
        result
        == "Ошибка при чтении CSV: [Errno 2] No such file or directory: 'empty.csv'"
    )
...
def test_invalid_json(mock_json_load, mock_file):
    ...
    assert (
        "Ошибка при чтении .json: Expecting value: line 1 column 2 (char 1)" in result
    )
...
def test_json_conversion_error(mock_read_csv, mock_to_json):
    ...
    assert (
        "Ошибка при чтении CSV: [Errno 2] No such file or directory: 'test.csv'"
        in result
    )
